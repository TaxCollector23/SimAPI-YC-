...
const items: Item[] = [
  { status: "shipped", title: "Dimensional-analysis validation engine", detail: "core/dimensional/ — units resolution, Pi-group discovery, majority-corruption-proof anchored constants, and a Pi-space response-surface layer. ~25 hand-specified rules with domain coverage from a units dictionary; replaced the old ~470-885-check hand-written engine (core/physics_validator.py, kept live only as a non-authoritative legacy comparison field). Live on the deployed site via the self-hosted Python backend." },
  ...
  { status: "shipped", title: "Hybrid TypeScript/Python engine on the deployed site", detail: "PYTHON_API_URL points the deployed site at the self-hosted Render backend, so the playground and API run the full dimensional-analysis engine. Falls back to a ~20-check TypeScript engine only if that backend is unreachable. The engine badge in the dashboard tells you which one you're getting." },
