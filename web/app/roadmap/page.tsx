...
const items: Item[] = [
  { status: "shipped", title: "1700+ deterministic physics checks across 21 domains", detail: "core/physics_validator.py — the full engine, live on the deployed site via the self-hosted Python backend." },
  ...
  { status: "shipped", title: "Hybrid TypeScript/Python engine on the deployed site", detail: "PYTHON_API_URL points the deployed site at the self-hosted Render backend, so the playground and API run the full 1700+ check engine. Falls back to a ~20-check TypeScript engine only if that backend is unreachable. The engine badge in the dashboard tells you which one you're getting." },
